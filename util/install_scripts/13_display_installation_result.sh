...
if [ $conn_mdb -eq 0 ]; then
	...
	MESSAGE="
Congratulation! Keexybox installed successfully.

KeexyBox Web interface is accessible by following addresses:
...
"
	...
	if [ "$use_whiptail" = "yes" ];then
		whiptail --backtitle "Keexybox installation" --title "Installation result" --msgbox "$MESSAGE" 15 50
	else
		echo "-------------------------------------------------"
		echo "$MESSAGE"
		echo "-------------------------------------------------"
